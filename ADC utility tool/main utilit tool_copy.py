...
class SIFT_Worker(QThread):
    ...
    def run(self):
        ...
        while len(self.image_dict) > 1:
            similar_group = []
            #importing images and path from dict
            path_list = list(self.image_dict.keys())
            image_list = list(self.image_dict.values())

            #using first image for compare
            img1 = image_list[0]
            img_name = path_list[0]
            self.Ref_image_path = img_name
            del self.image_dict[path_list[0]]

            #finding similar group
            #t_clock1 = datetime.now()
            similar_group = self.SIFT_Akaze_algo(img1, self.image_dict, groups, similar_group)
            ...
            self.Print_log_item(" ".join(["there is : ", str(len(self.image_dict)), " images left"]) ,"black")
