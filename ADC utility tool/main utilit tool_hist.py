...
class SIFT_Worker(QThread):
    ...
    def compute_similarity_matrix(self,sift_im_dict):
        image_keys = list(sift_im_dict.keys())
        n = len(image_keys)
        similarity_matrix = np.zeros((n, n))
        bf = cv2.BFMatcher()
        for i in range(n):
            self.Print_log_item(" ".join(["comparing image: ", str(n-i), " out of ",str(n)]), "black")
            for j in range(i + 1, n):
                _, desc1 = sift_im_dict[image_keys[i]]
                _, desc2 = sift_im_dict[image_keys[j]]
    ...
    def cluster_images(self,similarity_matrix, eps, min_samples,spatial_threshold):
        # Normalize the similarity matrix
        normalized_matrix = similarity_matrix / np.max(similarity_matrix)

        # Apply DBSCAN clustering
        dbscan = DBSCAN(eps=eps, min_samples=min_samples, metric='precomputed')
        clusters = dbscan.fit_predict(1 - normalized_matrix)

        # Merge clusters based on spatial proximity
        merged_clusters = {}
        for i, cluster_label in enumerate(clusters):
            if cluster_label != -1:  # Ignore noise points
                if cluster_label not in merged_clusters:
                    merged_clusters[cluster_label] = []
                merged_clusters[cluster_label].append(i)

        final_clusters = {}
        visited_clusters = set()

        for cluster_label, cluster_indices in merged_clusters.items():
            if cluster_label not in visited_clusters:
                visited_clusters.add(cluster_label)
                merged_cluster = cluster_indices

                for other_label, other_indices in merged_clusters.items():
                    if other_label not in visited_clusters:
                        if self.are_clusters_close(cluster_indices, other_indices, similarity_matrix,
                                                   spatial_threshold):
                            merged_cluster.extend(other_indices)
                            visited_clusters.add(other_label)

                if len(merged_cluster) >= min_samples:
                    final_clusters[cluster_label] = merged_cluster

        # Assign cluster labels to each image
        final_labels = np.full(len(similarity_matrix), -1)
        for cluster_label, cluster_indices in final_clusters.items():
            for idx in cluster_indices:
                final_labels[idx] = cluster_label

        return final_labels

    def are_clusters_close(self, cluster1, cluster2, similarity_matrix, threshold):
        avg_similarity = 0
        count = 0
        for i in cluster1:
            for j in cluster2:
    ...
    def cluster_images2(self,similarity_matrix, eps, min_samples):
        # Normalize the similarity matrix
        normalized_matrix = similarity_matrix / np.max(similarity_matrix)
    ...
    def group_images_by_cluster(self,sift_im_dict, clusters):
        image_keys = list(sift_im_dict.keys())
        cluster_groups = {}
        for i, cluster_label in enumerate(clusters):
            if cluster_label not in cluster_groups:
                cluster_groups[cluster_label] = []
            cluster_groups[cluster_label].append(image_keys[i])
    ...
    def run(self):
        ...
        similar_images = self.find_similar_images_hist(self.image_dict, similarity_threshold)

        # Group similar images into clusters
        cluster_groups = {}
        for image_path, similarity in similar_images:
            if similarity not in cluster_groups:
                cluster_groups[similarity] = []
            cluster_groups[similarity].append(image_path)

        indx = 0
        str_tmp = 'group_'
        new_dict = {}
        for similarity, image_list in cluster_groups.items():
            new_dict[str_tmp + str(indx)] = [image_list]
            indx += 1

        time.sleep(0.001)
        similar_images_dict["no_feature"] = self.no_featur_list
        self.found_similar_group.emit(new_dict)
        self.Print_log_item(
            " ".join(["Comparison done, found: ", str(len(similar_images_dict)), " groups of similar images"]), "green")

    def find_similar_images_hist(self, image_dict, similarity_threshold):
        similar_images = []
        self.Print_log_item("Comparison done, found:", "green")
        for query_image_path, query_image in image_dict.items():
            self.Print_log_item("Comparison done, found:")
            query_hist = self.calculate_image_histogram(query_image)

            for image_path, image in image_dict.items():
                if image_path == query_image_path:
                    continue

                hist = self.calculate_image_histogram(image)

                similarity = cv2.compareHist(query_hist, hist, cv2.HISTCMP_INTERSECT)
    ...
    def calculate_image_histogram(self, image, bins=8):
        hist = cv2.calcHist([image], [0, 1, 2], None, [bins, bins, bins], [0, 256, 0, 256, 0, 256])
        hist = cv2.normalize(hist, hist).flatten()
        return hist
    def find_similar_images(self, image_dict, similarity_threshold):
        similar_images = []

        for query_image_path, query_features in image_dict.items():
            query_keypoints, query_descriptors = query_features

            query_hist = self.calculate_descriptor_histogram(query_descriptors)

            for image_path, image_features in image_dict.items():
                if image_path == query_image_path:
                    continue

                image_keypoints, image_descriptors = image_features

                hist = self.calculate_descriptor_histogram(image_descriptors)
    ...
    def calculate_descriptor_histogram(self, descriptors, bins=8):
        if descriptors is None or len(descriptors) == 0:
            return np.zeros((bins, bins, bins))

        hist = cv2.calcHist([descriptors], [0, 1, 2], None, [bins, bins, bins], [0, 256, 0, 256, 0, 256])
        hist = cv2.normalize(hist, hist).flatten()
